...
if (html2code($input->get('action'))=='uploadword') {
    ...
    $html = readWordToHtml(str_replace(SYS_UPLOAD_URL, SYS_UPLOAD_PATH, $cache_file['url']), $config['module'], $config['userid'], $config['catid'], $config['siteid'], $config['is_wm'], $config['attachment'], $config['image_reduce'], md5(FC_NOW_URL.$input->get_user_agent().$input->ip_address().$config['userid']));
