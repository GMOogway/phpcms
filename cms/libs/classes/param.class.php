	...
	public static function set_cookie($var, $value = '', $time = 0) {
		...
		if (is_array($value)) {
			foreach($value as $k=>$v) {
				setcookie($var.'['.$k.']', sys_auth($v, 'ENCODE', md5(PC_PATH.'cookie'.$var).SYS_KEY), $time, COOKIE_PATH, COOKIE_DOMAIN, $s, $httponly);
			}
		} else {
			setcookie($var, sys_auth($value, 'ENCODE', md5(PC_PATH.'cookie'.$var).SYS_KEY), $time, COOKIE_PATH, COOKIE_DOMAIN, $s, $httponly);
		}
	}
