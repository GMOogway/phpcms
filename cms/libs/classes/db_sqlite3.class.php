...
final class db_sqlite3 {
	...
	public function connect() {
		try {
			$this->link = (!$this->config['password'])
				? new SQLite3(CMS_PATH.$this->config['database'])
				: new SQLite3(CMS_PATH.$this->config['database'], SQLITE3_OPEN_READWRITE | SQLITE3_OPEN_CREATE, $this->config['password']);
			$this->link->busyTimeout(15 * 1000); // 设置繁忙延迟时间
		} catch (Exception $e) {
			$this->halt('读取数据库文件失败：' . iconv('gbk', 'utf-8', $e->getMessage()));
		}
		return $this->link;
	}
	...
	public function list_tables() {
		$tables = array();
		$this->execute("select * from sqlite_master where type='table' order by name");
		while($r = $this->fetch_next()) {
			//if($r['name']=='sqlite_sequence') continue;
			$tables[] = $r['name'];
		}
		return $tables;
	}
	...
	public function version() {
		...
		return $version['versionString'];//server_version
	}
