<?php 
	...
	function set_config($config, $filename="system") {
		...
		foreach($config as $k=>$v) {
			if(in_array($k,array('js_path','css_path','img_path','app_path','mobile_js_path','mobile_css_path','mobile_img_path','mobile_path','bdmap_api','sys_editor','sys_max_category','sys_admin_pagesize','admin_founders','timezone','sys_time_format','sys_csrf','sys_csrf_time','needcheckcomeurl','admin_log','tpl_edit','gzip','debug','errorlog','cookie_pre','auth_key','connect_enable', 'upload_url','sina_akey', 'sina_skey', 'qq_appid','qq_appkey','qq_callback','keywordapi','xunfei_aid','xunfei_skey','baidu_aid','baidu_skey','baidu_arcretkey','baidu_qcnum'))) {
				$v = trim($v);
				$configs[$k] = $v;
