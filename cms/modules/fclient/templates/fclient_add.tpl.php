...
<!--
	...
	$("#fclient_name").formValidator({onshow:"<?php echo L("input").L('name')?>",onfocus:"<?php echo L("input").L('name')?>"}).inputValidator({min:1,onerror:"<?php echo L("input").L('name')?>"}).ajaxValidator({type : "get",url : "",data :"m=fclient&c=fclient&a=public_name",datatype : "html",async:'false',success : function(data){if( data == "1" ){return true;}else{return false;}},buttons: $("#dosubmit"),onerror : "<?php echo L('name').L('exists')?>",onwait : "<?php echo L('connecting')?>"}).defaultPassed();
...
<tr>
		<th></th>
		<td><input type="hidden" name="forward" value="?m=fclient&c=fclient&a=add"> <input
