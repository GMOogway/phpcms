...
<p class="gray4"><?php echo L('create_man')?>：<a href="#" class="blue"><?php echo $info['username']?></a>， <?php echo L('create_time')?>：<?php echo format::date($info['createtime'], 1)?></p>
	...
	<td align="center"><span style="height:22"><a href='?m=special&c=content&a=init&specialid=<?php echo $info['id']?>'><?php echo L('manage_news')?></a></span> | <span style="height:22"><a href='javascript:import_c(<?php echo $info['id']?>);void(0);'><?php echo L('import_news')?></a></span><br />
...
<span style="height:22"><a href="###" onclick="Dialog.confirm('<?php echo L('confirm', array('message'=>addslashes(new_html_special_chars($info['title']))))?>',function(){redirect('?m=special&c=special&a=delete&id=<?php echo $info['id']?>&pc_hash='+pc_hash);});"><?php echo L('del_special')?></a></span><br/>
<span style="height:22"><a href='?m=special&c=template&specialid=<?php echo $info['id']?>' style="color:red" target="_blank"><?php echo L('template_manage')?></a></span></td>
	</tr>
<?php 
	}
}
