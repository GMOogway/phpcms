...
switch($field_type) {
	...
	case 'pages':
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` ADD `paginationtype` TINYINT( 1 ) NOT NULL DEFAULT '0' $tips");
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` ADD `maxcharperpage` MEDIUMINT( 6 ) NOT NULL DEFAULT '0' $tips");
		} else {
	...
	case 'readpoint':
		$defaultvalue = intval($defaultvalue);
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` ADD `readpoint` smallint(5) unsigned NOT NULL default '$defaultvalue' $tips");
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` ADD `paytype` tinyint(1) unsigned NOT NULL default '0' $tips");
		} else {
