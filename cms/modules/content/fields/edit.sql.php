...
switch($field_type) {
	case 'varchar':
		...
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, $sql);
		} else {
			$this->db->query($sql);
		}
	break;
	...
	case 'smallint':
		...
		$defaultvalue = intval($defaultvalue);
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` SMALLINT ".($minnumber >= 0 ? 'UNSIGNED' : '')." NOT NULL DEFAULT '$defaultvalue' $tips");
		} else {
	...
	case 'pages':
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` CHANGE `paginationtype` `paginationtype` TINYINT( 1 ) NOT NULL DEFAULT '0' $tips");
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` CHANGE `maxcharperpage` `maxcharperpage` MEDIUMINT( 6 ) NOT NULL DEFAULT '0' $tips");
		} else {
	...
	case 'readpoint':
		$defaultvalue = intval($defaultvalue);
		if (!$issystem && $modelid && $modelid!=-1 && $modelid!=-2) {
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` CHANGE `$oldfield` `readpoint` smallint(5) unsigned NOT NULL default '$defaultvalue' $tips");
			sql_module($content_data['id'], $tablename, "ALTER TABLE `$tablename` CHANGE `paytype` `paytype` tinyint(1) unsigned NOT NULL default '0' $tips");
		} else {
