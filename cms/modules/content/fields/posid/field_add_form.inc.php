
	<div class="form-group">
      <label class="col-md-2 control-label">每个位置宽度</label>
      <div class="col-md-9">
            <label><input type="text" name="setting[width]" value="125" size="5" class="input-text"></label>
            <span class="help-block"><?php echo L('px')?></span>
      </div>
    </div>
	<div class="form-group">
      <label class="col-md-2 control-label">默认选中项</label>
      <div class="col-md-9">
            <label><input type="text" name="setting[defaultvalue]" value="" size="20" class="input-text"></label>
            <span class="help-block"><?php echo L('多个之间用半角逗号隔开')?></span>
      </div>
    </div>
